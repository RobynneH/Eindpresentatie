...
def boxplot_listing_price():
    ...
    fig = px.box(coords, x='house_type', y='price', hover_data=['address', 'living_area', 'room', 'bedroom', 'bathroom'],
                 labels={'house_type': 'House Type', 'price': 'Price (in Euros)'},
                 title='Interactive Boxplot of Current Listing Prices by House Type in Amsterdam',
...
def energie_labels():
    ...
    fig = px.scatter(coords_filtered, x='year_built', y='price', color='energy_label',
                     ...
                     labels={'year_built': 'Year Built', 'price': 'Price (in Euros)'},
                     title='Scatter Plot of Listing Prices by Year Built in Amsterdam',
                     width=800, height=600,
                     category_orders={'energy_label': ['A+', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'na']})
    fig.update_layout(legend_title='Energy Label')
...
#     # You can also display additional content using Streamlit
...
with col5:
    map_amsterdam()
with col6:
    st.write("Map met koopwoningen in Amsterdam.")
