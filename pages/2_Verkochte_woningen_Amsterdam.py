...
# vertaal de kolomnamen naar het Nederlands
Funda_reg_na.columns = ["oppervlakte", "bouwjaar", "kamers", "slaapkamers", "badkamers", "toiletten", "energie_label", 
                        "prijs_verkocht", "laatst_gevraagde_prijs_m2", "datum_verkocht"]
...
def boxplot_prijs():
    ...
    fig.update_layout({'updatemenus':[{'active':0, 'buttons':dropdown_buttons}]}, 
                       title = "Prijs per verkocht woning of laatst gevraagde prijs per vierkante meter",
                         yaxis_title = 'Woningen in Amsterdam')
    st.plotly_chart(fig)
...
def  boxplot_soort():
    ...
    fig.add_trace(go.Box(y = FS_woonboot["price_sold"], name = "Verkochte prijs per woonboot"))
    fig.add_trace(go.Box(y = FS_bovenwoning["price_sold"], name = "Verkochte prijs per bovenwoning"))
    fig.add_trace(go.Box(y = FS_benedenwoning["price_sold"], name = "Verkochte prijs per benedenwoning"))
    fig.add_trace(go.Box(y = FS_maisonnette["price_sold"], name = "Verkochte prijs per maissonnette"))
    fig.add_trace(go.Box(y = FS_portiek["price_sold"], name = "Verkochte prijs per portiekhuis"))
    ...
    fig.update_layout(title = "Prijs per soort verkochte woning",
                     showlegend = False,
                     xaxis_title = "Soort woning",
                     yaxis_title = "Verkochte prijs")
    st.plotly_chart(fig)
    
def hulpbron():
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Histogram(histfunc = "count", x = Funda_overeen["neighborhood_name"], name = "Aantal woningen"),
                  secondary_y = False)
    ...
    fig.update_layout(title = "Aantal verkochte huizen per wijk in Amsterdam en de veiligheidsindex per categorie", 
                      xaxis_title = "Wijknaam", height = 800, width = 1100)
    fig.update_yaxes(title_text="Aantal woningen", secondary_y=False)
...
def heatmap():
    ...
    fig, ax = plt.subplots()
    correlaties = Funda_reg_na.corr()
    sns.heatmap(correlaties[['prijs_verkocht']].sort_values(by="prijs_verkocht", ascending = True), annot = True, ax = ax)
    st.pyplot(fig)
    
def bereken_r2():
    # splits de dataframe op in de inputvariabelen en targetkolom
    y = correlaties['prijs_verkocht']
    X = correlaties.drop('prijs_verkocht', axis = 1)
...
def lin_reg():
    ...
    plt.scatter(Funda_reg_na['oppervlakte'], Funda_reg_na['prijs_verkocht'])
    m, b = np.polyfit(Funda_reg_na['oppervlakte'], Funda_reg_na['prijs_verkocht'], 1)
    plt.plot(Funda_reg_na['oppervlakte'].values, m*Funda_reg_na['oppervlakte'].values + b, c='k')
    plt.ticklabel_format(style = 'plain', axis = 'y')
    plt.xlabel('Oppervlakte van de woning in m2')
    plt.ylabel('Prijs per woning in Amsterdam')
    plt.title("Lineair Regressie model tussen de prijs en oppervlakte van een woning in Amsterdam")
    st.pyplot(plt)
...
with col1:
    boxplot_prijs()
with col2:
    st.write("Deze boxplot laat de verdeling van de prijzen van verkochte woningen zien. Met de button kan je de boxplot wijzigen naar de verdeling van laatst gevraagde prijs per vierkante meter. Hierbij is bij beide te zien dat de IQR niet heel breed is, en dat de prijzen dus best veel van elkaar verschillen. Ook is het duidelijk te zien dat de kwart met de duurste woning prijzen een groot verschil hebben. Hier zijn duidelijk een paar uitwijkers te zien.")
    st.write("De gemiddelde woningprijs is 485k euro, en de gemiddelde laatst gevraagde prijs per vierkante meter is 7353 euro.")
...
with col5:
    boxplot_soort()
with col6:
    st.write("In de boxplots hiernaast zijn de prijzen per type woning te zien. Hier is een groot verschil te zien tussen de prijzen van bijvoorbeeld een flat en de prijzen van een grachtenpand of herenhuis. Verder is er te zien dat per type woning de prijzen groot verschillen. dit kan zijn door bijvoorbeeld ligging of grootte van de woning.")
...
with col7:
    st.write("De Gemeente Amsterdam heeft per wijk verschillende veiligheids-categorieën opgesteld met een index. Dit zijn: ")
    ...
    st.write(f"- **Slachtofferschap index:** Deze index is gebaseerd op vragen uit de Veiligheidsmonitoren. Hierin zit high impact slachtofferschap, veelvoorkomend slachtofferschap en digitale criminaliteit.")
    st.write(f"- **Overlastindex:** Deze index is gebaseerd op overlastmeldingen van bewoners. Hieronder valt jeugdoverlast, geluidsoverlast, rommel op straat en onderhoud straatverlichting.")
    ...
    st.write("")
    st.write("In de grafiek hebben we de aantal verkochte woningen per wijk van hoog naar laag gesoorteerd om te kijken of de indexcijfers van laag naar hoog zouden komen te staan. Het is te zien dat de waardes aan de rechterkant van de grafiek duidelijk hoger liggen dan links. Dit betekent dat onze verwachting klopt en dat er minder woningen verkocht worden in wijken waar meer overlast en/of criminaliteits is.")
with col8:
    hulpbron()
...
with col9:
    heatmap()
with col10:
    st.write("Met de heatmap kunnen we de correlaties zien van een aantal variabelen op de prijs van een woning. Hierbij is te zien dat de oppervlakte van een woning de grootste correlatie heeft met de prijs van een woning. Dit wordt dan ook de variabele die wij gaan gebruiken voor ons lineair model.")
